...
export default function Vehicle({ checkpoint }) {
  ...
  const carBodyGeometry = new BoxGeometry(width, height, front * 2);
  const carBodyMaterial = new MeshStandardMaterial({ color: '#ff0000' });
  const carBody = new Mesh(carBodyGeometry, carBodyMaterial);
  carBody.castShadow = true;
  carBody.receiveShadow = true;

  const carTopGeometry = new BoxGeometry(width * 0.8, height * 0.5, front);
  const carTopMaterial = new MeshStandardMaterial({ color: '#cc0000' });
  const carTop = new Mesh(carTopGeometry, carTopMaterial);
  carTop.position.set(0, height * 0.75, 0);
    ...
    <>
      <group ref={vehicle} name="vehicle">
        <group ref={chassisBody} matrixWorldNeedsUpdate={true}>
          <primitive object={car} position={[0, -0.7, -0.1]} />
